...
let CancelCreditService = class CancelCreditService {
    ...
    async verifyCancelCredit(cancelCredit) {
        ...
        if (c) {
            let r = await this.registration.findOne({ student: c.student, course: c.course }).populate;
            if (r) {
                const index = r.subject.findIndex(item => item._id.equals(ObjectId));
                console.log(r);
                r.subject.map();
            }
            return c;
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("f79b23db66cbdebf71d8")
/******/ })();
/******/ 
/******/ }
;