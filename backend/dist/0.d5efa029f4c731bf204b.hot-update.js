...
let CancelCreditService = class CancelCreditService {
    ...
    async verifyCancelCredit(cancelCredit) {
        ...
        if (c) {
            let s = await this.subject.findOne({ _id: cancel });
            let r = await this.registration.findOne({ student: c.student, course: c.course }).populate('subject');
            if (r) {
                console.log(r.subject);
                const index = r.subject.findIndex(item => item._id.equals(cancelCredit.cancelCreditId));
                r.subject.splice(index, 1);
            }
            return c;
        }
        else
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("4a3b6fa731ebd0dbca99")
/******/ })();
/******/ 
/******/ }
;